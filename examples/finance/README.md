...
| `settled_value >= 0.0` | 3 | inductive; assumption `payment.amount >= 0.0` is a **generated runtime guard** |
| `exposure >= 0.0` | 3 | same |
...
It is an `f64` accumulated from thousands of concurrent messages. Rust has no
atomic float, so the hand-written equivalent is `Arc<Mutex<f64>>` — lock
ordering, hot-path contention, and the ever-present risk of holding a guard
across an `.await`. Here the value lives inside one actor's task and is
returned at `join()`; sharing never arises.

## Measured under 20% fault injection

```
[Ingest]     admitted = 1920   handled + dropped = 1920 + 0
[RiskGate]   passed   = 1920   exposure      = 1920.0
[Clearing]   netted   = 1920
[Settlement] settled  = 1920   settled_value = 1920.0
